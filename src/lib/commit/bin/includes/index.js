...
async function setupGit() {
  ...
  if (!remotes.some(remote => remote.name === "origin")) {
    console.log("⚠️ No remote found. Adding remote...");
    await git.addRemote("origin", process.env.SECRET_GIT_REMOTE_URL);
  }

  try {
    await git.checkout(process.env.SECRET_BRANCH_NAME);
  } catch (error) {
    console.log(`⚠️ Branch ${process.env.SECRET_BRANCH_NAME} not found. Creating it...`);
    await git.checkoutLocalBranch(process.env.SECRET_BRANCH_NAME);
  }
...
async function makeCommit() {
  try {
    ...
    if (!fs.existsSync(NOTES_FILE)) {
      fs.writeFileSync(NOTES_FILE, "Programming Tips:\n");
    }
    ...
    const currentLines = fs.readFileSync(NOTES_FILE, "utf8").split("\n");
    ...
    await git.add(NOTES_FILE);
    await git.commit(finalLine);
    await git.pull("origin", process.env.SECRET_BRANCH_NAME);
    await git.push("origin", process.env.SECRET_BRANCH_NAME);
